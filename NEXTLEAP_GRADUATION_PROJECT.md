...
**Tech Stack:**
- Python 3.11+, `pytest`, `.env` for API keys, Claude/GPT API.
...
**Tech Stack:**
- Python scrapers: PRAW (Reddit), google-play-scraper, app-store-scraper; JSONL + SQLite storage.
...
**Tech Stack:**
- `pandas` for cleaning; LLM relevance classifier (Claude/GPT) + embedding-based dedup.
...
**Tech Stack:**
- LLM extraction (Claude/GPT) with structured JSON (`pydantic`); embeddings + vector store (ChromaDB).
...
**Tech Stack:**
- `scikit-learn`/`hdbscan` for clustering; `pandas` for quantification tables.
...
**Tech Stack:**
- Streamlit dashboard; SQLite evidence DB; reports as Markdown/HTML.

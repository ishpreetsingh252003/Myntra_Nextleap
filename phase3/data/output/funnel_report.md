# Phase 3 Filtering Funnel Report

- **Generated:** 2026-08-26T02:14:34
- **Run:** p3_20260825-204434
- **Classifier:** rules-v1.0 (rules only, offline)

**Totals:** collected=30 -> cleaned=24 -> dedup-kept=22 -> relevant=18

Exact duplicates removed:
1
Near duplicates removed:
1

## Per-source funnel

| Source | Collected | Quarantined | Cleaned | Dedup-kept | Relevant | Relevant % |
|--------|-----------|-------------|--------|-----------|----------|-------------|
| amazon | 2 | 1 | 1 | 1 | 1 | 50% |
| app_store | 5 | 1 | 4 | 4 | 4 | 80% |
| forums_blogs | 4 | 2 | 2 | 2 | 2 | 50% |
| google_play | 4 | 1 | 3 | 3 | 1 | 25% |
| product_reviews | 4 | 1 | 3 | 3 | 3 | 75% |
| quora | 1 | 0 | 1 | 1 | 1 | 100% |
| reddit_web | 6 | 0 | 6 | 4 | 3 | 50% |
| youtube_comments | 4 | 0 | 4 | 4 | 3 | 75% |

## Quarantine breakdown (never deleted silently, EC-06/13/14/04/07)

- `out_of_scope_language`: 1
- `spam`: 4
- `too_short`: 1
